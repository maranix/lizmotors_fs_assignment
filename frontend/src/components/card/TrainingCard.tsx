...
const TrainingCard: React.FC<Props> = ({ className, variant = "compact" }) => {
	return variant === "compact" ? <CompactCard className={className} /> : <ExpandedCard />;
};

const CompactCard: React.FC<CardProps> = ({ className = "" }) => {
	return (
		<article className={mergeStyles("p-4 bg-card rounded-md shadow-md space-y-4", className)}>
			<h1 className="text-xl font-bold">Personal Protective Equipment (PPE)</h1>
			<p className="text-text-secondary">
				Explore the importance, types, and best practices for using and maintaining protective gear.
			</p>
			<StageProgressBar />
		</article>
	);
...
const StageProgressBar: React.FC = () => {
	...
	return (
		<div className="flex flex-row justify-between items-center text-sm">
			<div id="progress-container" className="flex flex-row items-center space-x-2">
				<div id="training-progress-bar" className="h-2 w-24 bg-secondary-light rounded-md">
					<div className="h-2 bg-primary rounded-md" style={{ width: `${completionPercentage}%` }} />
				</div>
